...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    switch (req.body.grant_type) {
        ...
        case "password":
            ...
            const launcherTicketUser = await discordLauncherAuth.consumeLaunchTicket(email, password);
            if (launcherTicketUser) {
                req.user = launcherTicketUser;
                break;
            }

            const regex = /@projectcube\.dev$/;
            const cubeAccount = regex.test(email);
            log.debug(`CUBE account: ${cubeAccount}`);
            if (cubeAccount && config.bEnableCubeUser) {
                ...
                if (findUser) {
                    ...
                    const registerUser = await functions.registerUser(numberWith8Digits, `cube_${email.split("@")[0]}`, email, password, true);
        ...
        if (memory.build != config.bVersionJoinable) {
            ...
            return error.createError(
                "errors.com.epicgames.version_not_supported",
                    "This version is blocked, change bSeasonJoinable or disable bEnableOnlyOneSeasonJoinable in the CUBE config.",
                [], -1, undefined, 400, res
            );
        }
    }
...
app.get("/account/api/oauth/verify", verifyToken, (req, res) => {
    ...
    if (config.bEnableOnlyOneVersionJoinable === true) {
        if (memory.build != config.bVersionJoinable) {
            ...
            return error.createError(
                "errors.com.epicgames.version_not_supported",
                    "This version is no longer supported, change bSeasonJoinable or disable bEnableOnlyOneSeasonJoinable in the CUBE config.",
                [], -1, undefined, 400, res
            );
        }
    }
...
app.post("/auth/v1/oauth/token", async (req, res) => {
    res.json({
                access_token: "cubetokenoniichan",
        token_type: "bearer",
        ...
        features: [
            ...
            "Ecom"
                ...
                organization_id: "cubetokenoniichan",
        product_id: "prod-fn",
        sandbox_id: "fn",
                deployment_id: "cubedeploymentidokasanbaka",
        expires_in: 3599
    });
})
...
app.post("/epic/oauth/v2/token", async (req, res) => {
    ...
    res.json({
        ...
        token_type: "bearer",
            access_token: "cubetokenoniichan",
            refresh_token: "cuberefreshtokenotosan",
            id_token: "cubeidtokenbaka",
        expires_in: 7200,
        expires_at: "9999-12-31T23:59:59.999Z",
        ...
        account_id: req.user.accountId,
        client_id: clientId,
            application_id: "cubesacpplicationidaishteimasu",
