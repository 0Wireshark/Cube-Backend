...
let systemCloudFilesCache = null;
let systemCloudFilesSignature = "";

function getSystemCloudFiles() {
    const dir = path.join(__dirname, "..", "CloudStorage");
    if (!fs.existsSync(dir)) fs.mkdirSync(dir);

    const iniFiles = fs.readdirSync(dir).filter(name => name.toLowerCase().endsWith(".ini")).sort();
    const signature = iniFiles.map((name) => {
        const stats = fs.statSync(path.join(dir, name));
        return `${name}:${stats.mtimeMs}:${stats.size}`;
    }).join("|");

    if (systemCloudFilesCache && systemCloudFilesSignature === signature) {
        return systemCloudFilesCache;
    }

    systemCloudFilesSignature = signature;
    systemCloudFilesCache = iniFiles.map(name => {
...
app.get("/fortnite/api/cloudstorage/system/:file", (req, res) => {
    ...
    if (fs.existsSync(file)) return res.status(200).sendFile(file);
