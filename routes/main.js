...
const cloudDirManifest = fs.readFileSync(
  path.join(__dirname, "..", "responses", "CloudDir", "LawinServer.manifest"),
);
const cloudDirChunk = fs.readFileSync(
  path.join(__dirname, "..", "responses", "CloudDir", "LawinServer.chunk"),
);
const cloudDirIni = fs.readFileSync(
  path.join(__dirname, "..", "responses", "CloudDir", "Full.ini"),
);
const eventListTemplate = JSON.parse(
  fs.readFileSync(path.join(__dirname, "./../responses/eventlistactive.json"), "utf-8"),
);
...
app.post("/fortnite/api/game/v2/chat/*/*/*/pc", (req, res) => {
  ...
  let resp = config.chat.EnableGlobalChat
    ? { GlobalChatRooms: [{ roomName: "cubebackendglobal" }] }
    : {};
...
app.get("/launcher/api/public/assets/*", async (req, res) => {
  res.json({
    ...
    labelName: "CubeBackend",
    buildVersion: "++Fortnite+Release-20.00-CL-19458861-Windows",
    catalogItemId: "5cb97847cee34581afdbc445400e2f77",
    expires: "9999-12-31T23:59:59.999Z",
    items: {
      MANIFEST: {
        signature: "CubeBackend",
        distribution: "https://cubebackend.ol.epicgames.com/",
        path: "Builds/Fortnite/Content/CloudDir/CubeBackend.manifest",
      ...
      CHUNKS: {
        signature: "CubeBackend",
        distribution: "https://cubebackend.ol.epicgames.com/",
        path: "Builds/Fortnite/Content/CloudDir/CubeBackend.chunk",
        additionalDistributions: [],
      },
    },
...
app.get("/Builds/Fortnite/Content/CloudDir/*.manifest", async (req, res) => {
  ...
  res.status(200).send(cloudDirManifest).end();
});

app.get("/Builds/Fortnite/Content/CloudDir/*.chunk", async (req, res) => {
  res.set("Content-Type", "application/octet-stream");
  res.status(200).send(cloudDirChunk).end();
});
...
app.get("/Builds/Fortnite/Content/CloudDir/*.ini", async (req, res) => {
  res.status(200).send(cloudDirIni).end();
});
...
app.get("/api/v1/events/Fortnite/download/:accountId", async (req, res) => {
  ...
  try {
    ...
    const events = JSON.parse(JSON.stringify(eventListTemplate));
    ...
    log.error("Error fetching Arena data:", error);
...
async function handleDatarouterData(req, res) {
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      if (findUser) {
        const playlist = global.kv ? await global.kv.get(`playerPlaylist:${accountId}`) : undefined;
        try {
          await playerStats.recordDatarouterEvents(accountId, data, playlist);
        } catch (statsError) {
          log.error("Career stats update failed:", statsError);
        }

        // Batch all operations to avoid multiple DB calls
        let totalKills = 0;
        ...
        if (totalKills > 0) {
          // Add hype points for all kills at once
          operations.push(
            addEliminationHypePointsBatch(findUser, totalKills)
          );
          // Add V-Bucks for all kills at once
          operations.push(
            mtxRewards(findUser, totalKills * 25, false)
          );
          log.debug(`Added ${totalKills} elimination hype points and ${totalKills * 25} v-bucks for user: ${accountId}`);
        }

        if (hasVictory) {
          operations.push(addVictoryHypePoints(findUser));
          operations.push(mtxRewards(findUser, 50, false));
          log.debug(`Added victory hype points and 50 v-bucks for user: ${accountId}`);
        }

        if (hasDeath) {
          operations.push(deductBusFareHypePoints(findUser));
        }
...
app.post([
  ...
  "/telemetry/data/datarouter/api/v1/public/data",
  "/telemetry/data/datarouter/api/v1/public/data/clients"
], handleDatarouterData);
