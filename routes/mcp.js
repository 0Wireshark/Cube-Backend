...
global.profileRefresh = global.profileRefresh || {};
...
function createAccountNotFound(accountId, res) {
...
function createGiftDisabled(accountId, direction, res) {
  return error.createError(
    "errors.com.epicgames.user.gift_disabled",
    `User ${accountId} has disabled ${direction} gifts.`,
    [accountId],
    28004,
    undefined,
    403,
...
function createFriendshipTooNew(senderId, receiverId, waitSeconds, res) {
  return error.createError(
    "errors.com.epicgames.friends.friendship_too_new",
    `User ${senderId} can not gift ${receiverId} yet.`,
    [senderId, receiverId, waitSeconds],
    28004,
    undefined,
    403,
    res,
...
const BULK_SPECIAL_COSMETICS = new Set([
...
function buildTemplateIdIndex(items) {
  const index = new Map();

  for (const [itemId, item] of Object.entries(items || {})) {
    const templateId = normalizeTemplateId(item?.templateId);
    if (templateId && !index.has(templateId)) index.set(templateId, itemId);
  }

  return index;
}

function buildTemplateIdSet(items) {
  return new Set(
    Object.values(items || {})
...
function findMtxCurrencyItem(profile) {
  const currentPlatform = normalizeTemplateId(profile?.stats?.attributes?.current_mtx_platform);

  for (const [itemId, item] of Object.entries(profile?.items || {})) {
    if (!normalizeTemplateId(item?.templateId).startsWith("currency:mtx")) continue;

    const platform = normalizeTemplateId(item?.attributes?.platform);
...
function getBulkEquipEntries(body) {
  if (Array.isArray(body?.loadoutData)) return body.loadoutData;
  if (Array.isArray(body?.slots)) return body.slots;
  if (Array.isArray(body?.items)) return body.items;
  if (Array.isArray(body?.customizations)) return body.customizations;
  if (body?.slotName || body?.category) return [body];
  return [];
}

function findAthenaItemIdByTemplate(profile, templateId, templateIdIndex) {
  const normalizedTemplateId = normalizeTemplateId(templateId);
  if (!normalizedTemplateId) return "";

  if (templateIdIndex) {
    return templateIdIndex.get(normalizedTemplateId) || "";
  }

  for (const [itemId, item] of Object.entries(profile.items || {})) {
    if (normalizeTemplateId(item?.templateId) === normalizedTemplateId) {
      return itemId;
    }
  }
...
function resolveAthenaItem(profile, itemToSlot, templateIdIndex) {
  if (!itemToSlot) {
  ...
  if (profile.items?.[itemToSlot]) {
    return {
      itemId: itemToSlot,
      templateId: profile.items[itemToSlot].templateId,
    };
  }

  return {
    itemId: findAthenaItemIdByTemplate(profile, itemToSlot, templateIdIndex),
    templateId: itemToSlot,
  };
}

function getActiveLoadoutId(profile, fallbackLockerItem) {
  if (fallbackLockerItem && profile.items?.[fallbackLockerItem]) {
    return fallbackLockerItem;
  }

  const attributes = profile.stats?.attributes || {};
  const loadouts = Array.isArray(attributes.loadouts) ? attributes.loadouts : [];
  return (
    loadouts[attributes.active_loadout_index] ||
    attributes.last_applied_loadout ||
    loadouts[0] ||
    "sandbox_loadout"
  );
}

function getFavoriteAttributeName(category) {
  if (category === "ItemWrap") return "favorite_itemwraps";
  return `favorite_${category}`.toLowerCase();
}

function buildVariantPayload(profile, itemId, variantUpdates) {
  const itemVariants = Array.isArray(variantUpdates)
    ? variantUpdates
  ...
  return {
    variants: itemVariants
      .filter((variant) => variant?.channel && variant?.active)
      .map((variant) => ({
...
function applyVariantUpdates(profile, itemId, variantUpdates, ApplyProfileChanges) {
  if (!itemId || !profile.items?.[itemId] || !Array.isArray(variantUpdates)) {
    return false;
  }

  if (!profile.items[itemId].attributes) profile.items[itemId].attributes = {};
  if (!Array.isArray(profile.items[itemId].attributes.variants)) {
    profile.items[itemId].attributes.variants = [];
  }

  let changed = false;
  for (const variant of variantUpdates) {
    if (!variant || typeof variant !== "object" || !variant.channel || !variant.active) {
      continue;
    }

    const variants = profile.items[itemId].attributes.variants;
    const index = variants.findIndex((entry) => entry.channel === variant.channel);

    if (index === -1) {
      variants.push({
        channel: variant.channel,
        active: variant.active,
        owned: [variant.active],
      });
      changed = true;
      continue;
    }

    if (variants[index].active !== variant.active) {
      variants[index].active = variant.active;
      changed = true;
    }

    if (!Array.isArray(variants[index].owned)) variants[index].owned = [];
    if (!variants[index].owned.includes(variant.active)) {
      variants[index].owned.push(variant.active);
      changed = true;
    }
  }
...
function applyBulkCosmeticEntry(profile, entry, ApplyProfileChanges, defaultLockerItem, templateIdIndex) {
  const category = entry?.slotName || entry?.category;
  if (!COSMETIC_SLOT_NAMES.includes(category)) {
    return false;
  }

  const itemToSlot = String(entry.itemToSlot ?? entry.item ?? entry.templateId ?? "");
  const { itemId, templateId } = resolveAthenaItem(profile, itemToSlot, templateIdIndex);
  const normalizedTemplateId = normalizeTemplateId(templateId);

  if (!templateId && ["Character", "Pickaxe", "Glider"].includes(category)) {
    return false;
  }

  if (
    templateId &&
    !normalizedTemplateId.startsWith(`athena${category.toLowerCase()}:`) &&
    !BULK_SPECIAL_COSMETICS.has(normalizedTemplateId)
  ) {
    return false;
  }

  const loadoutId = getActiveLoadoutId(profile, entry.lockerItem || defaultLockerItem);
  const loadout = profile.items?.[loadoutId];
  const slot = loadout?.attributes?.locker_slots_data?.slots?.[category];
  if (!slot) {
    return false;
  }

  applyVariantUpdates(profile, itemId, entry.variantUpdates, ApplyProfileChanges);
  const activeVariants = buildVariantPayload(profile, itemId, entry.variantUpdates);
  const favoriteValue = itemId || itemToSlot;
  let changed = false;

  if (category === "Dance" || category === "ItemWrap") {
    const slotCount = category === "ItemWrap" ? 7 : 6;
    const slotIndex = Number.isInteger(entry.indexWithinSlot) ? entry.indexWithinSlot : entry.slotIndex;
    const favoriteName = getFavoriteAttributeName(category);
    if (!Array.isArray(profile.stats.attributes[favoriteName])) {
      profile.stats.attributes[favoriteName] = new Array(slotCount).fill("");
    }

    if (slotIndex === -1 && category === "ItemWrap") {
      slot.items = new Array(slotCount).fill(templateId);
      profile.stats.attributes[favoriteName] = new Array(slotCount).fill(favoriteValue);
      slot.activeVariants = new Array(slotCount).fill(activeVariants);
      changed = true;
    } else if (Number.isInteger(slotIndex) && slotIndex >= 0 && slotIndex < slotCount) {
      while (slot.items.length < slotCount) slot.items.push("");
      slot.items[slotIndex] = templateId;
      profile.stats.attributes[favoriteName][slotIndex] = favoriteValue;
      if (!Array.isArray(slot.activeVariants)) slot.activeVariants = new Array(slotCount).fill(null);
      slot.activeVariants[slotIndex] = activeVariants;
      changed = true;
    }
  } else {
    const favoriteName = getFavoriteAttributeName(category);
    slot.items = [templateId];
    profile.stats.attributes[favoriteName] = favoriteValue;

    if (activeVariants) {
      slot.activeVariants = [activeVariants];
    } else if (category === "Character" || category === "Backpack") {
      slot.activeVariants = [{ variants: [] }];
    } else if (category === "Pickaxe") {
      slot.activeVariants = [];
    } else if (["LoadingScreen", "MusicPack", "SkyDiveContrail"].includes(category)) {
      slot.activeVariants = [null];
    }
  ...
  ApplyProfileChanges.push({
    ...
    attributeValue: loadout.attributes.locker_slots_data,
  });

  ApplyProfileChanges.push({
    changeType: "statModified",
    name: getFavoriteAttributeName(category),
    value: profile.stats.attributes[getFavoriteAttributeName(category)],
  });
...
app.post(
  ...
  verifyToken,
  async (req, res) => {
    log.debug(`GiftCatalogEntry: Request received for offer ${req.body?.offerId || ""}`);
    ...
    if (!profile0) {
      return createAccountNotFound(req.user.accountId, res);
    }

    if (!gifting.canSendGifts(profile)) {
      log.debug(`GiftCatalogEntry: User ${req.user.accountId} has disabled sending gifts`);
      return createGiftDisabled(req.user.accountId, "sending", res);
    }
    ...
    let ProfileRevisionCheck =
      memory.build >= 12.2 ? profile.commandRevision : profile.rvn;
    let QueryRevision = req.query.rvn || -1;
    let senderProfile0Changed = false;
    let validGiftBoxes = [
      "GiftBox:gb_default",
    ...
    if (!validGiftBoxes.includes(req.body.giftWrapTemplateId) && req.body.giftWrapTemplateId !== "") {
      log.debug(
    ...
    for (let receiverId of req.body.receiverAccountIds) {
      ...
      const relationship = gifting.validateGiftRelationship(sender, req.user.accountId, receiverId);
      if (relationship.reason == "friendship_too_new") {
        log.debug(
          `GiftCatalogEntry: User ${req.user.accountId} can not gift ${receiverId} yet`,
        );
        return createFriendshipTooNew(req.user.accountId, receiverId, relationship.waitSeconds, res);
      }
    ...
    if (
      ...
      return error.createError(
        "errors.com.epicgames.fortnite.id_invalid",
        `Offer ID (id: '${req.body.offerId}') is not giftable`,
        [req.body.offerId],
        16027,
        undefined,
        400,
    ...
    switch (true) {
      case /^BR(Daily|Weekly|Special(Featured|Daily)|Featured)Storefront$/.test(findOfferId.name):
      case findOfferId.offerId?.giftInfo?.bIsEnabled === true: {
        const receiverProfileDocs = await Profile.find({
          accountId: { $in: req.body.receiverAccountIds },
        });
        const receiverProfilesById = new Map(
          receiverProfileDocs.map((document) => [document.accountId, document]),
        );
        receiverProfilesById.set(req.user.accountId, profiles);

        for (let receiverId of req.body.receiverAccountIds) {
          const receiverProfiles = receiverProfilesById.get(receiverId);
          if (!receiverProfiles) {
            return createAccountNotFound(receiverId, res);
          }

          const athena = receiverProfiles.profiles?.athena;
          const common_core = receiverProfiles.profiles?.common_core;
          if (!athena || !common_core) {
            return createAccountNotFound(receiverId, res);
          }

          if (!athena.items) athena.items = {};
          const ownedAthenaTemplates = buildTemplateIdSet(athena.items);

          if (!gifting.canReceiveGifts(common_core)) {
            log.debug(
              `GiftCatalogEntry: User ${receiverId} has disabled receiving gifts`,
            );
            return createGiftDisabled(receiverId, "receiving", res);
          }

          for (let itemGrant of findOfferId.offerId.itemGrants) {
            if (ownedAthenaTemplates.has(normalizeTemplateId(itemGrant.templateId))) {
              log.debug(
        ...
        if (
          ...
          const currency = findMtxCurrencyItem(profile);

          if (currency && currency.item.quantity >= price) {
            profile.items[currency.itemId].quantity -= price;
            if (profile0.items?.[currency.itemId]) {
              profile0.items[currency.itemId].quantity -= price;
              senderProfile0Changed = true;
            }

            ApplyProfileChanges.push({
              changeType: "itemQuantityChanged",
              itemId: currency.itemId,
              quantity: profile.items[currency.itemId].quantity,
            });
            if (senderProfile0Changed) {
              ApplyProfileChanges.push({
                changeType: "itemQuantityChanged",
                itemId: currency.itemId,
                quantity: profile0.items[currency.itemId].quantity,
              });
            }

            log.debug(
              `GiftCatalogEntry: Currency deducted: ${price}, remaining ${profile.items[currency.itemId].quantity}`,
            );
          } else if (price > 0) {
            const available = currency?.item?.quantity;
            log.debug(
              `GiftCatalogEntry: Insufficient currency: required ${price}, available ${available ?? "none"}`,
            );
            return error.createError(
              "errors.com.epicgames.currency.mtx.insufficient",
              available === undefined
                ? `You can not afford this item.`
                : `You can not afford this item (${price}), you only have ${available}.`,
              available === undefined ? [] : [`${price}`, `${available}`],
              1040,
              undefined,
              400,
              res,
        ...
        for (let receiverId of req.body.receiverAccountIds) {
          ...
          let giftBoxItem = {
            templateId: req.body.giftWrapTemplateId || "GiftBox:gb_default",
            attributes: {
          ...
          for (let value of findOfferId.offerId.itemGrants) {
            ...
            const Item = athenaInventory.createAthenaItem(value.templateId);
          ...
          if (receiverId == req.user.accountId && senderProfile0Changed) {
          ...
          if (receiverId == req.user.accountId && senderProfile0Changed) {
            receiverUpdate[`profiles.profile0`] = profile0;
          }

          await receiverProfiles.updateOne({ $set: receiverUpdate });

          global.giftReceived[receiverId] = true;
          global.profileRefresh[receiverId] = {
            ...(global.profileRefresh[receiverId] || {}),
            athena: true,
          };
      ...
      default:
        return error.createError(
          "errors.com.epicgames.modules.gamesubcatalog.purchase_not_allowed",
          `Could not gift catalog offer ${findOfferId.offerId.devName}.`,
          [findOfferId.offerId.devName],
          28004,
          undefined,
          403,
          res,
    ...
    if (
      ...
      if (senderProfile0Changed) {
        senderUpdate[`profiles.profile0`] = profile0;
      }
...
app.post(
  ...
  verifyToken,
  async (req, res) => {
    log.debug(`PurchaseCatalogEntry: Request received for offer ${req.body?.offerId || ""}`);
...

    {
      ...
      var ItemExists = false;
      let BattlePass = functions.getBattlePass(playerSeason);
    ...
    switch (true) {
      case /^BR(Daily|Weekly|Season)Storefront$/.test(findOfferId.name):
        ...
        for (let value of findOfferId.offerId.itemGrants) {
          ...
          if (ownedAthenaTemplates.has(normalizeTemplateId(value.templateId))) {
            log.debug(
          ...
          athena.items[ID] = Item;
          ownedAthenaTemplates.add(normalizeTemplateId(Item.templateId));
        ...
        if (
          ...
          const price = findOfferId.offerId.prices[0].finalPrice;
          const currency = findMtxCurrencyItem(profile);
          ...
          if (currency && currency.item.quantity >= price) {
            ...
            ApplyProfileChanges.push(
              ...
              },
              {
                changeType: "itemQuantityChanged",
                itemId: currency.itemId,
                quantity: profile0.items?.[currency.itemId]?.quantity ?? profile.items[currency.itemId].quantity,
              },
            );

            log.debug(
              `PurchaseCatalogEntry: Currency deducted: ${price}, remaining ${profile.items[currency.itemId].quantity}`,
            );
          } else if (price > 0) {
            const available = currency?.item?.quantity;
            log.debug(
              `PurchaseCatalogEntry: Insufficient currency: required ${price}, available ${available ?? "none"}`,
            );
            return error.createError(
              "errors.com.epicgames.currency.mtx.insufficient",
              available === undefined
                ? `You cannot afford this item (${price}).`
                : `You cannot afford this item (${price}), you only have ${available}.`,
              available === undefined ? [`${price}`] : [`${price}`, `${available}`],
              1040,
              undefined,
              400,
              res,
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!profile.items) profile.items = {};
    const repairedProfile = athenaInventory.repairAthenaProfile(profile);
    ...
    if (profile.items[req.body.itemToSlot]) {
      ...
      if (
        ...
        let itemVariants = profile.items[req.body.itemToSlot].attributes.variants || [];
        let variantsObj = null;

        if (req.body.slotName === "Character" || req.body.slotName === "Backpack") {
          variantsObj = { variants: itemVariants.map(v => ({ channel: v.channel, active: v.active })) };
        } else if (itemVariants.length > 0) {
          variantsObj = { variants: itemVariants.map(v => ({ channel: v.channel, active: v.active })) };
        }

        if (req.body.slotName === "Dance" || req.body.slotName === "ItemWrap") {
          let idx = req.body.indexWithinSlot;
          if (typeof idx === "number" && idx >= 0) {
            if (!profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants) {
              profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = new Array(req.body.slotName === "ItemWrap" ? 7 : 6).fill(null);
            }
            profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants[idx] = variantsObj;
          } else if (req.body.slotName === "ItemWrap" && idx === -1) {
            profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = new Array(7).fill(variantsObj);
          }
        } else {
          if (variantsObj) {
            profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = [variantsObj];
          } else if (req.body.slotName === "Pickaxe") {
            profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = [];
          } else {
            profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = [null];
          }
        }
      }
    }
    ...
    if (ApplyProfileChanges.length > 0 || repairedProfile) {
      profile.rvn += 1;
      profile.commandRevision += 1;
      profile.updated = new Date().toISOString();
...
app.post(
  ...
  async (req, res) => {
    ...
    if (req.body.itemToSlot) {
      itemToSlotID = profile.items[req.body.itemToSlot]
        ? req.body.itemToSlot
        : findAthenaItemIdByTemplate(
            profile,
            req.body.itemToSlot,
            buildTemplateIdIndex(profile.items),
          );
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (req.query.profileId == "athena" && athenaInventory.repairAthenaProfile(profile)) {
      profile.rvn = (profile.rvn || 0) + 1;
      profile.commandRevision = (profile.commandRevision || 0) + 1;
      shouldSaveProfile = true;
    }
    ...
    const queuedProfileRefresh = global.profileRefresh[req.user.accountId];

    if (
      req.query.profileId == "common_core" &&
      (global.giftReceived[req.user.accountId] || queuedProfileRefresh?.athena)
    ) {
      global.giftReceived[req.user.accountId] = false;
      delete global.profileRefresh[req.user.accountId];

      let athena = profiles.profiles["athena"];

      if (athena) {
        if (athenaInventory.repairAthenaProfile(athena)) {
    ...
    switch (req.params.operation) {
      ...
      case "BulkEquipBattleRoyaleCustomization": {
        if (req.query.profileId != "athena") {
          return error.createError(
            "errors.com.epicgames.modules.profiles.invalid_command",
            `BulkEquipBattleRoyaleCustomization is not valid on ${req.query.profileId} profile`,
            ["BulkEquipBattleRoyaleCustomization", req.query.profileId],
            12801,
            undefined,
            400,
            res,
        ...
        const entries = getBulkEquipEntries(req.body);
        const defaultLockerItem = req.body?.lockerItem;
        const templateIdIndex = buildTemplateIdIndex(profile.items);
        let changed = false;
        for (const entry of entries) {
          changed =
            applyBulkCosmeticEntry(profile, entry, ApplyProfileChanges, defaultLockerItem, templateIdIndex) ||
            changed;
        }
...
app.post(
  ...
  async (req, res) => {
    ...
    // Le game server a toujours besoin du profil complet pour appliquer les cosmétiques
    ApplyProfileChanges = [
      {
        changeType: "fullProfileUpdate",
        profile: profile,
...
function checkFields(fields, body) {
  ...
  fields.forEach((field) => {
    if (body[field] === undefined || body[field] === null) missingFields.fields.push(field);
  });
