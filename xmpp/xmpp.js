...
gameServerHeartbeat.registerRoutes(app);

function getMatchmakerPort() {
    if (Number.isInteger(config.matchmakerPort) && config.matchmakerPort > 0 && config.matchmakerPort <= 65535) {
        return config.matchmakerPort;
    }

    if (typeof config.matchmakerIP === "string") {
        try {
            const url = new URL(
                config.matchmakerIP.startsWith("ws://") || config.matchmakerIP.startsWith("wss://")
                    ? config.matchmakerIP
                    : `${config.bEnableHTTPS ? "wss" : "ws"}://${config.matchmakerIP}`
            );

            const urlPort = Number(url.port);
            if (Number.isInteger(urlPort) && urlPort > 0 && urlPort <= 65535) return urlPort;
        } catch {}
    }

    return config.bEnableHTTPS ? 443 : 80;
}

const port = getMatchmakerPort();
let wss;
...
wss.on('connection', async (ws, req) => {
    ...
    if (ws.protocol.toLowerCase() != "xmpp") return matchmaker(ws, parseMatchmakerPayload(req));
...
function pickMatchmakerPayload(rawPayload) {
    const tokens = String(rawPayload || "").trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0) return null;

    return tokens.find(token => token.startsWith("v1."))
        || tokens.find(token => global.matchmakingTickets?.has(token))
        || tokens[tokens.length - 1];
}

function parseMatchmakerPayload(req) {
    const authHeader = req?.headers?.authorization;
    if (typeof authHeader === "string") {
        const match = authHeader.match(/Epic-MMS-Player\s+(.+)/i);
        if (match && match[1]) return pickMatchmakerPayload(match[1]);

        const payload = pickMatchmakerPayload(authHeader);
        if (payload?.startsWith("v1.") || global.matchmakingTickets?.has(payload)) return payload;
    }

    try {
        const parsedUrl = new URL(req?.url || "/", "ws://localhost");
        return pickMatchmakerPayload(parsedUrl.searchParams.get("ticket") || parsedUrl.searchParams.get("payload"));
    } catch {
        return null;
    }
}
